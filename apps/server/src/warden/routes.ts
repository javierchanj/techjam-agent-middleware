...
export interface WardenRouteDeps {
  ...
  archive?: { find(traceId: string): Promise<import("./types.js").WardenTrace | null> } | undefined;
...
export async function registerWardenRoutes(
  ...
  app.post("/api/warden/policy/template", async (request, reply) => {
    ...
    try {
      return { policy: await deps.control.applyTemplate(id) };
    } catch (error) {
      return reply
        .code(400)
  ...
  app.get("/api/warden/traces", async (request) => {
    ...
    const traces = await deps.control.listTraces(query.agentId);
    return {
